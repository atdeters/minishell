...
{
	if ((token->type == WORD || token->type ==DOUBLE_QOUTE || token->type ==DOLAR_SIGN || token->type ==SINGLE_QOUTE)
	 && (!token->prev || (token->prev && (token->prev->type != REDIR_APPEND 
		&& token->prev->type != REDIR_OUT && token->prev->type != REDIR_IN && token->prev->type != DELIMITER))))
			return (1);
	return (0);
}
...
int	check_for_putting_dollar(t_token *token)
{
	if (token->type == DOLAR_SIGN && (!token->prev || (token->prev
		&& (token->prev->type != REDIR_APPEND && token->prev->type != REDIR_OUT
			&& token->prev->type != REDIR_IN && token->prev->type != DELIMITER))))
				return (1);
	return (0);
}
