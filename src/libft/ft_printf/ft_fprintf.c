...
/*   Updated: 2024/11/15 19:42:35 by adeters          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_var_printer(int fd, char code, va_list list);
	...
	while (str[i])
	{
		if (str[i] == '%')
		{
			...
			if (ft_is_specifier(str[i]))
				bytes_written += ft_var_printer(fd, str[i++], args);
			else
...
static int	ft_var_printer(int fd, char code, va_list list)
{
	...
	if (code == 's')
	{
		...
		if (ptr != 0)
			return (ft_putstr_fd_retbytes(ptr, fd));
		else
		{
		...
		return (ft_putchars_fd(code, list, fd));
	else if (code == 'i' || code == 'd' || code == 'u')
		return (ft_putnumbers_fd(code, list, fd));
	else if (code == 'p' || code == 'x' || code == 'X')
		return (ft_puthexas_fd(code, list, fd));
	return (-1);
}
/* 
#include <stdio.h>
